...
from FC_config import TESTFEATURE_PATH
...
class CustomDataset(Dataset):
    def __init__(self, pid_path: Path, label_path: Path):
        ...
        cah_all: list = np.loadtxt(TESTFEATURE_PATH / "CAH_CSAR_36.lst", dtype='float').tolist()
        hydroS_all: list = np.loadtxt(TESTFEATURE_PATH / "HydroS_CSAR_36.lst", dtype='float').tolist()
        dash_all: list = np.loadtxt(TESTFEATURE_PATH / "DASH_CSAR_36.lst", dtype='float').tolist()
